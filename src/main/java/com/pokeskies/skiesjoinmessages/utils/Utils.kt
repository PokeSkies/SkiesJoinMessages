...
object Utils {
    ...
    fun parsePlaceholders(player: ServerPlayer, text: String): String {
        return SkiesJoinMessages.INSTANCE.placeholderManager.parse(player, text)
    }
    ...
    fun printDebug(message: String?, bypassCheck: Boolean = false) {
        if (bypassCheck || ConfigManager.CONFIG.debug)
            SkiesJoinMessages.LOGGER.info("[SkiesJoinMessages] DEBUG: $message")
    }

    fun printError(message: String?) {
        SkiesJoinMessages.LOGGER.error("[SkiesJoinMessages] ERROR: $message")
    }

    fun printInfo(message: String?) {
        SkiesJoinMessages.LOGGER.info("[SkiesJoinMessages] $message")
    }
